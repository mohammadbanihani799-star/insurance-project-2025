                    ...
                    <nav aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><a
                                    href="{{ route('super_admin.insurances-index') }}">Insurances</a></li>
                            <li class="breadcrumb-item active" aria-current="page">Insurance Details</li>
                        </ol>
                    </nav>
                </div>
            </div>
                    ...
                    <div class="card-body">
                        <center class="mt-4">
                            ...
                            @if (isset($insurance->image) && $insurance->image && file_exists($insurance->image))
                                <img src="{{ asset($insurance->image) }}" width="200" height="150" />
                            @else
                                <img src="{{ asset('style_files/shared/images_default/default.jpg') }}" width="200"  height="150" />
                            @endif

                            {{-- Insurance Type --}}
                            <small class="text-muted pt-4 db">Insurance Type</small>
                            <h4 class="card-title mt-2"> {{ isset($insurance->insurance_type) ? $insurance->insurance_type : '-------' }}</h4>

                            {{-- Status --}}
                            <small class="text-muted pt-4 db">Status</small>
                            <h6>
                                @if (isset($insurance->status) && $insurance->status == 'Active')
                                    <span style="color: green;"><strong>{{ $insurance->status }}</strong></span>
                                @elseif(isset($insurance->status) && $insurance->status == 'Inactive')
                                    <span style="color: red;"><strong>{{ $insurance->status }}</strong></span>
                                @else
                                    {{ isset($insurance->status) ? $insurance->status : '-------' }}
                                @endif
                            </h6>

                            {{-- Added Since --}}
                            <small class="text-muted pt-4 db">Added Since</small>
                            <h6>{!! isset($insurance->created_at) ? $insurance->created_at->diffForHumans() : '-------' !!}</h6>

                            {{-- Addition Time --}}
                            <small class="text-muted pt-4 db">Addition Time</small>
                            <h6>{!! isset($insurance->created_at) ? date('h:i A', strtotime($insurance->created_at)) : '-------' !!}</h6>

                            {{-- Addition Date --}}
                            <small class="text-muted pt-4 db">Addition Date</small>
                            <h6>{!! isset($insurance->created_at) ? date('Y / F (m) / d', strtotime($insurance->created_at)) : '-------' !!}</h6>
                        </center>
                    </div>
                </div>
            </div>
                    ...
                    <div class="tab-content" id="pills-tabContent">
                                        ...
                                        <div class="table-responsive">
                                            <table id="file_export_main_info_part_1"
                                                ...
                                                <thead>
                                                    {{-- id --}}
                                                    <tr>
                                                        <th style="background-color:aliceblue">رقم التأمين :</th>
                                                        <td>
                                                            <strong>
                                                                {{ isset($insurance->id) ? $insurance->id : '----' }}
                                                            </strong>
                                                        </td>
                                                    </tr>
                                                    ...
                                                    {{-- status --}}
                                                    <tr>
                                                        <th style="background-color:aliceblue">الحالة :</th>
                                                        <td>
                                                            <strong>
                                                                @if (isset($insurance->status) && $insurance->status == 'Active')
                                                                    <span
                                                                        style="color: green;"><strong>{{ isset($insurance->status) ? $insurance->status : '----' }}</strong></span>
                                                                @else
                                        ...
                                        <div class="table-responsive">
                                            <table id="file_export_status_team"
                                                ...
                                                <thead>
                                                    ...
                                                    <tr>
                                                        <th style="background-color:aliceblue">السعر:</th>
                                                        <td>
                                                            <strong>
                                                                {{ isset($insurance->price) ? $insurance->price . ' SAR' : '-------' }}
                                                            </strong>
                                                        </td>
                                                    </tr>
                        ...
                        <div class="tab-pane fade show fade" id="tab_body_2" role="tabpanel"
                            ...
                            <div class="card-body">
                                ...
                                <form action="{{ route('super_admin.insurances-addInsuranceBenefit', $insurance->id) }}" method="POST" id="signUpForm"
                            ...
                            <div class="card-body">
                                <div class="table-responsive">
                                    <table id="file_export" class="table table-striped table-bordered display">
                                        ...
                                        <tbody>
                                            @if (isset($insurance->insuranceBenefits) && $insurance->insuranceBenefits->count() > 0)
                                                @foreach ($insurance->insuranceBenefits as $insuranceBenefit)
                                                    <tr>
