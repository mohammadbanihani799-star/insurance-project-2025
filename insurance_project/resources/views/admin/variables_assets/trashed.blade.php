                    ...
                    <nav aria-label="breadcrumb">
                        ...
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page"><a
                                    href="{{ route('super_admin.variables_assets-index') }}">Vendors</a></li>
                            ...
                            <table id="file_export" class="table table-striped table-bordered display">
                                ...
                                <tbody>
                                    @foreach ($variablesAssets as $variableAsset)
                                        <tr>
                                            <td>{{ isset($variableAsset->title) ? $variableAsset->title : '----' }}</td>
                                            <td>{{ isset($variableAsset->quantity) ? $variableAsset->quantity : '----' }}</td>
                                            <td>{{ isset($variableAsset->status) ? $variableAsset->status : '----' }}</td>
                                            <td>{!! isset($variableAsset->created_at)
                                                ? '<strong>Date : </strong>' .
                                                    date('Y -m-d', strtotime($variableAsset->created_at)) .
                                                    '<br><strong>Time : </strong>' .
                                                    date('h:i A', strtotime($variableAsset->created_at)) .
                                                    '<br><strong>Since : </strong> ' .
                                                    $variableAsset->created_at->diffForHumans()
                                                : "<span style='color:blue;'>----------</span>" !!}</td>
                                            <td>
                                                <div class="button-group">
                                                    <a href="{{ route('super_admin.variables_assets-softDeleteRestore', [isset($variableAsset->id) ? $variableAsset->id : -1]) }}"
                                            ...
                                            <td class="text-center">
                                                <input type="checkbox" class="selectedVariableAssets" name="selectedVariableAssets[]"
                                                    value="{{ $variableAsset->id }}">
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
    ...
    <script>
        function softDeleteRestoreSelected() {
            ...
            if (selectedVariableAssets.length > 0) {
                ...
                var link = '{{ route('super_admin.variables_assets-softDeleteRestoreSelected') }}' + query;
