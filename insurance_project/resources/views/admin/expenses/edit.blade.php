                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating mb-3">
                                        <input type="text" name="title"
                                            ...
                                            id="tb-title" value="{{ old('title') ? old('title') : (isset($expense->title) ? $expense->title : null) }}"
                                            placeholder="Title">
                                        <label for="tb-title">
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating mb-3">
                                        <input type="date" name="expense_date"
                                            ...
                                            id="tb-expense_date"
                                            value="{{  old('expense_date') ? old('expense_date') : (isset($expense->expense_date) ? $expense->expense_date : null) }}"
                                ...
                                <div class="col-md-6">
                                    <div class="form-floating mb-3">
                                        <input type="number" name="amount" min="0.01" step="0.001"
                                            ...
                                            id="tb-amount" value="{{ old('amount') ? old('amount') : (isset($expense->amount) ? $expense->amount : null) }}"
                                            placeholder="amount">
                                        <label for="tb-amount">
                                ...
                                <div class="col-md-6 mb-2">
                                    ...
                                    <a href="{{ asset($expense->expense_file) }}"
                                        class="view-file-link" target="_blank">View Expense File</a>
                                                ...
                                                @if (isset($expensesTypes) && $expensesTypes->count() > 0)
                                                    ...
                                                    @foreach ($expensesTypes as $expenseType)
                                                        <option value="{{ $expenseType->id }}"
                                                            {{old('category_id') ? (old('category_id') == $expenseType->id ? 'selected' : '') : ($expenseType->id == $expense->category_id ? 'selected' : '')}}
                                                           >
                                                ...
                                                @if (isset($expenseLocations) && $expenseLocations->count() > 0)
                                                    ...
                                                    @foreach ($expenseLocations as $expenseLocation)
                                                        <option value="{{ $expenseLocation->id }}"
                                                            {{old('location_id') ? (old('location_id') == $expenseLocation->id ? 'selected' : '') : ($expenseLocation->id == $expense->location_id ? 'selected' : '')}}>
                                                ...
                                                @if (isset($accounts) && $accounts->count() > 0)
                                                    ...
                                                    @foreach ($accounts as $account)
                                                        <option value="{{ $account->id }}"
                                                            {{old('account_id') ? (old('account_id') == $account->id ? 'selected' : '') : ($account->id == $expense->account_id ? 'selected' : '')}}>
                                                            {{ $account->title_ar ?? '------' }}
                                                            ({{ $account->title_en ?? '------' }})
                                                        </option>
                                                    @endforeach
                                                ...
                                                @if (isset($vendors) && $vendors->count() > 0)
                                                    ...
                                                    @foreach ($vendors as $vendor)
                                                        <option value="{{ $vendor->id }}"
                                                            {{old('vendor_id') ? (old('vendor_id') == $vendor->id ? 'selected' : '') : ($vendor->id == $expense->vendor_id ? 'selected' : '')}}>
                                                            {{ $vendor->name_en ?? '------' }}
                                                            ({{ $vendor->name_ar ?? '------' }})
                                                        </option>
                                                    @endforeach
                                                ...
                                                @if (isset($variableAssets) && $variableAssets->count() > 0)
                                                    ...
                                                    @foreach ($variableAssets as $variableAsset)
                                                        <option value="{{ $variableAsset->id }}"
                                                            {{old('asset_id') ? (old('asset_id') == $variableAsset->id ? 'selected' : '') : ($variableAsset->id == $expense->asset_id ? 'selected' : '')}}>
                                                            {{ $variableAsset->title ?? '------' }}
                                                        </option>
                                                    @endforeach
                                                @else
                                ...
                                <div class="col-md-12">
                                    <div class="mb-3">
                                        ...
                                        <textarea name="description" class="form-control border border-info @error('description') border-danger @enderror"
                                            rows="5" placeholder="Description">{{ old('description') ? old('description') : (isset($expense->description) ? $expense->description : null) }}</textarea>
                                    </div>
                                </div>
