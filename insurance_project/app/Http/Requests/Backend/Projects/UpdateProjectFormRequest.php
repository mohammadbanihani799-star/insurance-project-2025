...
class UpdateProjectFormRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        $project = Project::find(isset($this->id) ? $this->id : -1);
        ...
        ->with('department')
        ->pluck('id')->toArray();
        $statusArray = [1, 2, 3, 4, 5]; // 1 => In Queue, 2 => In Progress, 3 => Completed (Active), 4 => Completed (Closed), 5 => Closed
        ...
        $rules =  [
            ...
            'subscriptions_status' => ['required', 'numeric', 'integer', Rule::in($subscriptionsTypesArray)],
        ...
        $paidOrPartiallyPaidPayments = $project->projectPayments->whereIn('payment_status', ['Paid', 'Partially Paid']);
        if ($paidOrPartiallyPaidPayments && ($totalContracts !=  $project->total_contracts)) {
            $rules['total_contract_change'] = 'required';
        }
        $totalPayments = array_sum($this->input('payment_amount', []));
        if ($totalContracts != $totalPayments) {
            $rules['total_payment_table'] = 'required';
        }
        // In case the project is PAID
        if (isset($project) && $paidOrPartiallyPaidPayments->count() > 0) {
            $projectPayments = $paidOrPartiallyPaidPayments;
           if (count($project->projectPayments) != count($this->payment_amount)) {
                $rules['payments_amount_records_count'] = 'required';
           }
           foreach ($projectPayments as $key => $projectPayment) {
                if ($this->payment_amount[$key] != $projectPayment->payment_amount) {
                    $rules['payments_amount_value'] = 'required';
                }
           }
        }
    ...
    public function messages()
    {
        return [
           ...
           'total_contract_change' => 'You can not change the value of total contract',
